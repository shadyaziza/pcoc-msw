...
var authHandler = import_msw.http.post("*/api/auth/login", async ({ request }) => {
  const { email, password } = await request.json();
  await (0, import_msw.delay)();
...
// src/handlers/cities.ts
var listCitiesHandler = import_msw2.http.get(
  "/api/countries/:countryCode/cities",
  async ({ params }) => {
    const { countryCode } = params;
    await (0, import_msw2.delay)();
    ...
    return import_msw2.HttpResponse.json(list);
  }
);
...
var listCountriesHandler = import_msw3.http.get("/api/countries", async ({ request }) => {
  await (0, import_msw3.delay)();
  let countries = [];
  ...
  return import_msw3.HttpResponse.json(countries);
});
var CountriesHandler = [listCountriesHandler];

// src/handlers/guests.ts
var import_msw4 = require("msw");
var listGuestsHandler = import_msw4.http.get("/api/guests", async ({ request }) => {
  let queryParams;
  ...
  return import_msw4.HttpResponse.json(response, { status: 200 });
});
var getGuestHandler = import_msw4.http.get(
  "/api/guests/:id",
  async ({ params }) => {
    const { id } = params;
    await (0, import_msw4.delay)();
    ...
    if (guest) {
      return import_msw4.HttpResponse.json(guest, { status: 200 });
    } else {
...
var createGuestHandler = import_msw4.http.post("/api/guests", async ({ request }) => {
  ...
  return import_msw4.HttpResponse.json(guestOut, { status: 201 });
});
var updateGuestHandler = import_msw4.http.put(
  "/api/guests/:id",
  ...
  async ({ params, request }) => {
    ...
    switch (result.success) {
      case true:
        return import_msw4.HttpResponse.json(result.guest, { status: 200 });
      case false:
        return import_msw4.HttpResponse.json({ error: result.error }, { status: 422 });
    }
  }
);
var updateBulkGuestStatusHandler = import_msw4.http.patch(
  "/api/guests/bulk-status",
  async ({ request }) => {
    const { guestIds, status } = await request.json();
    await (0, import_msw4.delay)();
    const result = bulkUpdateStatus(guestIds, status);
    return import_msw4.HttpResponse.json(result, { status: 200 });
  }
);
var checkEmailHandler = import_msw4.http.get(
...
async function setupMockServer() {
  const worker = (0, import_browser.setupWorker)(...handlers);
  await worker.start({
